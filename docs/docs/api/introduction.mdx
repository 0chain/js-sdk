...
| `checkIfWasmLoaded` | Checks if the GoSDK WASM is initialized. Returns `true` if the WASM is loaded, `false` otherwise. |
| `updateWasmMode` | Updates the desired mode of the GoSDK WASM. |
| `getDesiredMode` | Returns the desired mode of the GoSDK WASM. |
