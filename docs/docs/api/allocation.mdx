...
{/* is ZCN or SAS and update code docs too // DOTODO */}
#### Parameters
...
const txnHash = await freezeAllocation({
  domain: 'mainnet.zus.network',
  wallet: activeWallet,
  allocationId: '8695f8af52cdfc1e66d879407a5278703579d31de7ce0e2cf1707fdec9cc14c3'
})
...
const txnHash = await cancelAllocation({
  domain: 'mainnet.zus.network',
  wallet: activeWallet,
...
#### Example  
```ts  
const txnHash = await updateAllocation({  
  domain,  
  wallet,  
...
const txnHash = await updateAllocationWithRepair({
  domain: 'mainnet.zus.network',
  wallet: activeWallet,
  allocationId: '8695f8af52cdfc1e66d879407a5278703579d31de7ce0e2cf1707fdec9cc14c3',
  size: desiredSize,
...
{/* ZCN or SAS and update code docs too // DOTODO: https://0chain.slack.com/archives/D06680GNC49/p1739211606792009  */}
#### Parameters
| Name | Type | Description | Default value |
|---|---|---|---|
| `domain` | `NetworkDomain` | Network domain for the allocation | |
...
const txnHash = await updateForbidAllocation({
  wallet: activeWallet,
  domain: 'mainnet.zus.network',
  allocationId: '8695f8af52cdfc1e66d879407a5278703579d31de7ce0e2cf1707fdec9cc14c3',
  forbidUpload: true,
...
type AllocStatus = {
   ...
   * - `broken`: The allocation is broken (missing data blocks) and it cannot be repaired anymore.
   */
...
:::note
Ensure to terminate workers using [`terminateWorkers`](#terminateworkers) when the allocation is no longer needed to free up resources.
:::

### terminateWorkers
Removes local upload workers for a particular allocation. This is useful when switching between allocations and workers for the old allocation are no longer needed.
...
:::note
Use this method to free up resources when switching between allocations when the previously active allocation is no longer needed.
:::
