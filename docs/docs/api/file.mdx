...
### multiUpload
Uploads multiple files in a batch. This method can also be used to resume a paused upload.

:::warning
Keep the batch size under 50 files for optimal performance.
:::
...
### downloadDirectory
Downloads files in a directory (non-recursive). This method supports downloading all files in a specified directory and provides progress updates via a callback function.
{/* confirm non recursive fact // DOTODO: https://0chain.slack.com/archives/D06ADNR2P4L/p1739223347776269 */}
#### Parameters
| Name | Type | Description | Default value |
|---|---|---|---|
| `domain` | `NetworkDomain` | Network domain for the download | |
...
| `expiration` | `number` | Expiration time of the auth ticket. Defaults to 0 (no expiration) | `0` |
...
{/* expiration unit // DOTODO: https://0chain.slack.com/archives/D06ADNR2P4L/p1739226063557459 */}
#### Return Type
```typescript
Promise<string>
```
